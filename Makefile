...
# ghcr.io/containeroo/cloudflare-operator-bundle:$VERSION and ghcr.io/containeroo/cloudflare-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= ghcr.io/containeroo/cloudflare-operator
...
.PHONY: run-gateway-api
run-gateway-api: manifests generate fmt vet ## Run a controller from your host.
	go run ./cmd/main.go --enable-gateway-api
...
ifeq (,$(shell which opm 2>/dev/null))
	...
	mkdir -p $(dir $(OPM)) ;\
	OS=$(shell go env GOOS) && ARCH=$(shell go env GOARCH) && \
	curl -sSLo $(OPM) https://github.com/operator-framework/operator-registry/releases/download/v1.63.0/$${OS}-$${ARCH}-opm ;\
	chmod +x $(OPM) ;\
	}
else
OPM = $(shell which opm)
